...
pub struct Fat32 {
    pub bpb: FatBPB,
    pub ebpb: Fat32EBPB,
    pub alloc_table: Fat32AllocTable,
    pub root_dir: Vec<FatDirEntry>,
    pub clusters: Vec<u8>,
}

impl Fat32 {
    pub fn new<P: AsRef<StdPath>>(path: P) -> Result<Fat32, Box<dyn StdError>> {
        // ファイルを読み込む
        ...
        let (ebpb, bytes) = Fat32EBPB::parse(&bytes)?;
        let (alloc_table, bytes) = Fat32AllocTable::parse(&bytes, &bpb)?;
        ...
        Ok(Fat32 { bpb, ebpb, alloc_table, root_dir, clusters: bytes.to_vec() })
    }
...
#[derive(Debug)]
pub struct Fat32AllocTable {
    table: Vec<u16>,
}

impl Fat32AllocTable {
    pub fn parse<'a>(bytes: &'a [u8], bpb: &FatBPB) -> Result<(Fat32AllocTable, &'a [u8]), Box<dyn StdError>> {
        // u32 キャスト
        ...
        Ok((Fat32AllocTable { table }, &bytes[fat_size as usize..]))
    }
